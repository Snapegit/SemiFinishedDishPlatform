...
<div :style='{"minHeight":"500px","padding":"20px 20px 40px","margin":"0px auto","borderColor":"#d3d3d3","background":"#fff","borderWidth":"0 1px","width":"980px","position":"relative","borderStyle":"solid"}'>
    <el-form
      ...
      label-width="150px"
    >
          <el-form-item :style='{"width":"100%","padding":"0px","margin":"0 0 20px","background":"none","display":"inline-block"}' label="投票名称" prop="toupiaomingcheng">
            <el-input v-model="ruleForm.toupiaomingcheng" 
                placeholder="投票名称" clearable :disabled=" false  ||ro.toupiaomingcheng"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"0px","margin":"0 0 20px","background":"none","display":"inline-block"}' label="投票图片" v-if="type!='cross' || (type=='cross' && !ro.toupiaotupian)" prop="toupiaotupian">
            <file-upload
            tip="点击上传投票图片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"width":"100%","padding":"0px","margin":"0 0 20px","background":"none","display":"inline-block"}' class="upload" v-else label="投票图片" prop="toupiaotupian">
                ...
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.toupiaotupian.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"0px","margin":"0 0 20px","background":"none","display":"inline-block"}'  label="投票票数" prop="toupiaopiaoshu">
            <el-select v-model="ruleForm.toupiaopiaoshu" placeholder="请选择投票票数" :disabled=" false  ||ro.toupiaopiaoshu" >
              <el-option
          ...
          <el-form-item :style='{"width":"100%","padding":"0px","margin":"0 0 20px","background":"none","display":"inline-block"}' label="投票时间" prop="toupiaoshijian">
              <el-date-picker
				  :disabled="true  ||ro.toupiaoshijian"
          ...
          <el-form-item :style='{"width":"100%","padding":"0px","margin":"0 0 20px","background":"none","display":"inline-block"}' label="信息备注" prop="xinxibeizhu">
            <el-input v-model="ruleForm.xinxibeizhu" 
                placeholder="信息备注" clearable :disabled=" false  ||ro.xinxibeizhu"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"0px","margin":"0 0 20px","background":"none","display":"inline-block"}' label="账号" prop="zhanghao">
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='toupiaotupian'){
              this.ruleForm.toupiaotupian = obj[o].split(",")[0];
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`toupiaojilu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
      ...
      onSubmit() {
			...
			obj.toupiaopiaoshu = parseFloat(obj.toupiaopiaoshu) + parseFloat(this.ruleForm.toupiaopiaoshu)
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('toupiaojilu/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(table+`/update`,obj).then(res => {});
								this.$http.post(`toupiaojilu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      toupiaotupianUploadChange(fileUrls) {
          this.ruleForm.toupiaotupian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
      },
    }
  };
</script>
