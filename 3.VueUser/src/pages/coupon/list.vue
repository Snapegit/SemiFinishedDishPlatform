...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0px","margin":"10px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","justifyContent":"","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      ...
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>生效时间：</div>
        <el-date-picker 
          v-model="timeRange"
          type="datetimerange"
          range-separator="至"
          ...
          start-placeholder="生效时间起始"
          end-placeholder="生效时间结束"
          value-format="yyyy-MM-dd HH:mm:ss">
        </el-date-picker>
      </el-form-item>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>过期时间：</div>
        <el-date-picker 
          v-model="timeRange"
          type="datetimerange"
          range-separator="至"
		  :style='{"padding":"0px 10px","borderColor":"#ddd","borderRadius":"4px","background":"#fff","borderWidth":"1px","width":"auto","lineHeight":"36px","borderStyle":"solid","justifyContent":"center","height":"36px","maxWidth":"220px"}'
          start-placeholder="过期时间起始"
          end-placeholder="过期时间结束"
          value-format="yyyy-MM-dd HH:mm:ss">
        </el-date-picker>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('coupon','新增')" :style='{"cursor":"pointer","border":"1px solid #ccc","padding":"0px 8px","margin":"0px 10px 0 0","color":"#666","borderRadius":"4px","background":"#f9f9f9","display":"inline-block","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/couponAdd')"><i v-if="true" :style='{"color":"#666","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
...
<script>
  export default {
    ...
    async created() {
		...
		this.selectOptionsList.push({name:'券类型',list:this.typeOptions,tableName: 'type',check: -1})
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.timeRange.length > 0) {
          searchWhere.startimestart = this.timeRange[0];
          searchWhere.startimeend = this.timeRange[1];
        }
        if (this.timeRange.length > 0) {
          searchWhere.endtimestart = this.timeRange[0];
          searchWhere.endtimeend = this.timeRange[1];
        }
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`coupon/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/couponDetail', query: params});
      },
