...
<div>
	...
	<div v-if="centerType" :style='{"padding":"20px","margin":"0px auto","borderColor":"#d3d3d3","borderRadius":"0px","background":"#fff","borderWidth":"0 1px","width":"980px","borderStyle":"solid"}'>
		<el-button size="mini" @click="backClick">返回</el-button>
	</div>
	<div class="list-preview" :style='{"padding":"0 20px 20px","margin":"0px auto","borderColor":"#d3d3d3","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","borderWidth":"0 1px","display":"flex","width":"980px","position":"relative","borderStyle":"solid","justifyContent":"space-between"}'>
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0px","margin":"10px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","justifyContent":"","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      ...
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>商品品牌：</div>
        <el-input v-model="formSearch.shangpinpinpai" placeholder="商品品牌" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>食材来源：</div>
        <el-input v-model="formSearch.shicailaiyuan" placeholder="食材来源" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>营养成分：</div>
        <el-input v-model="formSearch.yingyangchengfen" placeholder="营养成分" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 5px 0 0"}'>
	  ...
	  <el-button v-if=" true " :style='{"cursor":"pointer","border":"1px solid #712122","padding":"0px 8px","margin":"0px 10px 0 10px","color":"#fff","borderRadius":"4px","background":"linear-gradient(180deg, rgba(210,12,13,1) 0%, rgba(168,10,9,1) 100%)","display":"inline-block","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="getList(1, curFenlei)"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-search"></i>查询</el-button>
	  <el-button v-if="btnAuth('peicaixinxi','新增')" :style='{"cursor":"pointer","border":"1px solid #ccc","padding":"0px 8px","margin":"0px 10px 0 0","color":"#666","borderRadius":"4px","background":"#f9f9f9","display":"inline-block","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/peicaixinxiAdd')"><i v-if="true" :style='{"color":"#666","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="sort_view" :style='{"padding":"0 10px","margin":"10px 0 0","overflow":"hidden","textAlign":"right","background":"none","width":"100%","order":"4"}'>
		<el-button :style='{"padding":"2px 0px","margin":"0 5px","borderColor":"#eccc19","borderRadius":"0px","background":"none","borderWidth":"0px","borderStyle":"solid"}' @click="sortClick('price')">
			<span :style='{"margin":"0 2px 0 0","lineHeight":"20px","fontSize":"14px","color":"#c20c0c"}' class="icon iconfont icon-chujia13" v-if="sortType!='price'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"20px","fontSize":"14px","color":"#c20c0c"}' class="icon iconfont icon-jiantou06" v-else-if="sortType=='price'&&sortOrder=='desc'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"20px","fontSize":"14px","color":"#c20c0c"}' class="icon iconfont icon-jiantou07" v-else-if="sortType=='price'&&sortOrder=='asc'"></span>
		...
		<el-button :style='{"border":"0px solid #ddd","padding":"2px 0px","margin":"0 5px","borderRadius":"0px","background":"none"}' @click="sortClick('clicknum')">
			<span :style='{"margin":"0 2px 0 0","lineHeight":"20px","fontSize":"14px","color":"#c20c0c"}' class="icon iconfont icon-xiaoliang17" v-if="sortType!='clicknum'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"20px","fontSize":"14px","color":"#c20c0c"}' class="icon iconfont icon-jiantou06" v-else-if="sortType=='clicknum'&&sortOrder=='desc'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"20px","fontSize":"14px","color":"#c20c0c"}' class="icon iconfont icon-jiantou07" v-else-if="sortType=='clicknum'&&sortOrder=='asc'"></span>
	...
	<div class="list" :style='{"width":"calc(100% - 0px)","padding":"20px 0 0 0","margin":"0","background":"none","flex":"1","order":"6"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0 0 0 20px","overflow":"hidden","color":"#666","background":"none","width":"100%","fontSize":"14px","height":"auto"}'>
			<div :style='{"border":"0px solid #ddd","cursor":"pointer","padding":"0 0 10px","margin":"0 43px 20px 0","overflow":"hidden","background":"#fff","display":"inline-block","width":"140px","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<img @click.stop="imgPreView(item.shangpintupian)" :style='{"border":"1px solid #eee","width":"140px","objectFit":"cover","display":"block","height":"140px"}' v-if="item.shangpintupian && item.shangpintupian.substr(0,4)=='http'" :src="item.shangpintupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.shangpintupian?item.shangpintupian.split(',')[0]:''))" :style='{"border":"1px solid #eee","width":"140px","objectFit":"cover","display":"block","height":"140px"}' v-else :src="baseUrl + (item.shangpintupian?item.shangpintupian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"0 0px","borderColor":"#ddd","margin":"0px","overflow":"hidden","whiteSpace":"nowrap","color":"#333","borderWidth":"0 0 0px","width":"100%","lineHeight":"24px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"solid"}' class="name ">{{item.shangpinmingcheng}}</div>
				...
				<div :style='{"padding":"0 0px","margin":"0 5px 0 0","display":"inline-block"}'>
				  <span class="icon iconfont icon-zan10" :style='{"margin":"0 0px 0 0","lineHeight":"1.5","fontSize":"inherit","color":"inherit"}'></span>
				  <span class="text" :style='{"color":"#999","lineHeight":"1.5","fontSize":"inherit"}'>{{item.thumbsupnum}}</span>
				</div>
				<div :style='{"padding":"0 0px","margin":"0 5px 0 0","display":"inline-block"}'>
				...
				<div :style='{"padding":"0 0px","display":"inline-block"}' v-if="item.clicknum">
				  <span class="icon iconfont icon-liulan04" :style='{"margin":"0 0px 0 0","lineHeight":"1.5","fontSize":"inherit","color":"#999"}'></span>
				  <span class="text" :style='{"color":"#999","lineHeight":"1.5","fontSize":"inherit"}'>{{item.clicknum}}</span>
				</div>
			</div>
		</div>
...
<script>
  export default {
    ...
    async created() {
		...
		await this.$http.get('option/peicaifenlei/peicaifenlei').then(res => {
			if (res.data.code == 0) {
				this.peicaifenleiOptions = res.data.data;
				this.selectOptionsList.push({name:'配菜分类',list:this.peicaifenleiOptions,tableName: 'peicaifenlei',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.indexQueryCondition) {
          searchWhere.shangpinmingcheng = '%' + this.indexQueryCondition + '%';
        ...
        if (this.formSearch.peicaifenlei != '') searchWhere.peicaifenlei = this.formSearch.peicaifenlei;
        if (this.formSearch.shangpinpinpai != '') searchWhere.shangpinpinpai = '%' + this.formSearch.shangpinpinpai + '%';
        if (this.formSearch.shicailaiyuan != '') searchWhere.shicailaiyuan = '%' + this.formSearch.shicailaiyuan + '%';
        if (this.formSearch.yingyangchengfen != '') searchWhere.yingyangchengfen = '%' + this.formSearch.yingyangchengfen + '%';
        ...
        this.$http.get(`peicaixinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
...
<style rel="stylesheet/scss" lang="scss" scoped>
	...
	.category-3 .item {
		...
		transition: all 0s;
		border-radius: 4px;
		background: linear-gradient(180deg, rgba(254,254,254,1) 0%, rgba(247,247,247,1) 50%, rgba(241,241,241,1) 100%);
		width: auto;
		border-width: 1px;
		border-style: solid;
		text-align: left;
