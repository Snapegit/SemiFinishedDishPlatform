...
<div>
	...
	<div class="news-preview-pv" :style='{"padding":"10px 20px 20px","margin":"0px auto","borderColor":"#d3d3d3","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","borderWidth":"0 1px","display":"flex","width":"980px","position":"relative","borderStyle":"solid","justifyContent":"space-between"}'>
		<el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0px 0","margin":"0","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","float":"right","justifyContent":"flex-end","height":"auto"}'>
			...
			<el-button type="primary" @click="getNewsList(1)" :style='{"cursor":"pointer","border":"1px solid #712122","padding":"0px 10px","margin":"0 0px 0 0","color":"#fff","borderRadius":"0 4px 4px 0","background":"linear-gradient(180deg, rgba(210,12,13,1) 0%, rgba(168,10,9,1) 100%)","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}'>
				<span class="icon iconfont icon-chakan14" :style='{"color":"#fff","margin":"0 0px 0 0","fontSize":"16px"}'></span>
				搜索
			</el-button>
		</el-form>
		...
		<div class="category" :style='{"padding":"0","margin":"20px 0 0","borderColor":"#ddd","flexWrap":"wrap","background":"none","borderWidth":"0px 0","display":"flex","width":"100%","borderStyle":"solid","justifyContent":"flex-start","height":"auto"}'>
		  ...
		  <div v-for="(item,index) in categoryList" @click="categoryClick(index+1)" :key="index" class="item" :class="categoryIndex == index+1 ? 'active' : ''">{{item.typename}}</div>
				...
				<!-- 样式三 -->
		<div class="list3 index-pv1" :style='{"padding":"0","margin":"20px 0","color":"#999","flex":"1","width":"calc(100% - 0px)","fontSize":"14px","height":"auto"}'>
			<div :style='{"cursor":"pointer","padding":"0 0 10px","margin":"0 0 10px","borderColor":"#eee","background":"none","borderWidth":"0 0 1px","width":"100%","position":"relative","borderStyle":"solid","height":"auto"}' v-for="item in newsList" :key="item.id" @click="toNewsDetail(item)" class="list-item animation-box">
				<div :style='{"overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"40px","fontSize":"14px","textOverflow":"ellipsis","fontWeight":"600"}' class="title ">{{item.title}}</div>
				<div :style='{"border":"0px solid #eee","padding":"0px","margin":"0 0 10px","color":"#666","background":"none","lineHeight":"24px","fontSize":"14px"}' class="introduction ">{{item.introduction}}</div>
				...
				<div :style='{"padding":"0 10px","margin":"10px 0 0","color":"#b59e1c","textAlign":"center","background":"#eccc1930","display":"none","width":"80px","fontSize":"14px","lineHeight":"32px","float":"right"}' class="tags ">新闻动态</div>
			</div>
		</div>
		...
		<div class="hot" :style='{"width":"100%","padding":"0","margin":"20px 0 0 0","background":"none","height":"auto","order":"60"}'>
		  <div :style='{"padding":"0 0 0 34px","borderColor":"#C10D0C","color":"#222","borderRadius":"0px","textAlign":"left","background":"url(http://codegen.caihongy.cn/20240124/c18108dd98c34cdbab5fc0b1ed2dd781.png) no-repeat 10px center","borderWidth":"0 0 2px","width":"100%","lineHeight":"34px","fontSize":"20px","borderStyle":"solid","fontWeight":"500"}'>热门推荐</div>
