...
<div>
	<div :style='{"padding":"20px","margin":"0px auto","borderColor":"#d3d3d3","borderRadius":"0px","background":"#fff","borderWidth":"0 1px","width":"980px","borderStyle":"solid"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'='" :style='{"fontSize":"14px","lineHeight":"1","justifyContent":"flex-start","display":"flex"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/peicaifenlei"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"padding":"0","margin":"0","flexWrap":"wrap","background":"none","flex":"1","display":"flex","width":"calc(100% - 230px)","position":"relative","justifyContent":"space-between","order":"2"}'>
			...
			<div class="info" :style='{"padding":"0px","margin":"0","borderRadius":"0px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","order":"2"}'>
				...
				<div class="btn" :style='{"width":"100%","padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"1px solid #009cf530","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#009cf5","borderRadius":"0px","background":"none","width":"auto","lineHeight":"34px","fontSize":"14px","height":"34px"}' v-if="btnAuth('peicaifenlei','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"1px solid #f5340030","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#f53400","borderRadius":"0px","background":"none","width":"auto","lineHeight":"34px","fontSize":"14px","height":"34px"}' v-if="btnAuth('peicaifenlei','删除')" @click="delClick">删除</el-button>
					<el-button :style='{"border":"1px solid #f5340030","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#f53400","borderRadius":"0px","background":"none","width":"auto","lineHeight":"34px","fontSize":"14px","height":"34px"}' v-if="btnAuth('peicaifenlei','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
				</div>
			</div>
		</div>
    ...
    methods: {
      ...
      sizeChange(size){
		...
		backClick(){
			...
			this.$router.push({path: '/index/peicaifenlei', query: params});
		},
		// 下载
		download(file){
			if(!file) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/peicaifenleiAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此配菜分类？')
			  .then(_ => {
			    this.$http.post('peicaifenlei/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
						this.$message({
							type: 'success',
							message: '删除成功!',
