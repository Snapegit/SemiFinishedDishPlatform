<template>
	<div class="addEdit-block" :style='{"padding":"85px 30px 30px","fontSize":"14px","color":"#666","background":"none"}'>
		...
		>
			<template >
				<el-form-item :style='{"border":"0px solid #eee","padding":"0px 0","margin":"0 0 20px 0","color":"inherit","borderRadius":"0px","background":"none","width":"100%","fontSize":"inherit"}' class="input" v-if="type!='info'"  label="配菜分类" prop="peicaifenlei">
					<el-input v-model="ruleForm.peicaifenlei" placeholder="配菜分类" clearable  :readonly="ro.peicaifenlei"></el-input>
				</el-form-item>
				<el-form-item :style='{"border":"0px solid #eee","padding":"0px 0","margin":"0 0 20px 0","color":"inherit","borderRadius":"0px","background":"none","width":"100%","fontSize":"inherit"}' v-else class="input" label="配菜分类" prop="peicaifenlei">
					<el-input v-model="ruleForm.peicaifenlei" placeholder="配菜分类" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
	data() {
		return {
			...
			rules: {
				peicaifenlei: [
					{ required: true, message: '配菜分类不能为空', trigger: 'blur' },
				],
			},
		};
	},
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `peicaifenlei/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.peicaifenleiCrossAddOrUpdateFlag = false;
											this.parent.search();
