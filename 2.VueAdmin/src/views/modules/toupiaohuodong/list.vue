<template>
	<div class="main-content" :style='{"color":"#3B3E40","padding":"30px 30px 30px","fontSize":"14px"}'>
			...
			<el-form class="center-form-pv" :style='{"border":"0px solid #bababa","padding":"0px","margin":"0px","overflow":"hidden","borderRadius":"0px","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"1px solid #e3e3e3","padding":"20px","borderRadius":"5px","textAlign":"center","flexWrap":"wrap","background":"#fff","flexDirection":"row","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"flex-start"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">投票名称</label>
						<el-input v-model="searchForm.toupiaomingcheng" placeholder="投票名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">发布时间</label>
						<el-date-picker v-model="searchForm.fabushijianstart" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" format="yyyy-MM-dd HH:mm:ss" placeholder="发布时间起始"></el-date-picker>
						至
						<el-date-picker v-model="searchForm.fabushijianend" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" format="yyyy-MM-dd HH:mm:ss" placeholder="发布时间结束"></el-date-picker>
					</div>
				...
				<el-row class="actions" :style='{"border":"0px solid #e3e3e3","padding":"0px","margin":"20px 0 10px","color":"#fff","textAlign":"left","flexDirection":"row","display":"flex","justifyContent":"flex-start","borderRadius":"5px","flexWrap":"wrap","background":"none","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('toupiaohuodong','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('toupiaohuodong','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('toupiaohuodong','投票票数统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"border":"0px solid #e9e9e9","width":"100%","padding":"0","margin":"10px 0 0","borderRadius":"0px","background":"rgba(255,255,255,1)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="toupiaomingcheng"
						label="投票名称">
						<template slot-scope="scope">
							{{scope.row.toupiaomingcheng}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="toupiaotupian" width="200" label="投票图片">
						<template slot-scope="scope">
							<div v-if="scope.row.toupiaotupian">
								<img v-if="scope.row.toupiaotupian.substring(0,4)=='http'" :src="scope.row.toupiaotupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.toupiaotupian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.toupiaotupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.toupiaotupian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('toupiaohuodong','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('toupiaohuodong','投票')" @click="toupiaojiluCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','已投票操作！')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								投票
							</el-button>
							<el-button class="edit" v-if=" isAuth('toupiaohuodong','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai10" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('toupiaohuodong','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('toupiaohuodong','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<toupiaojilu-cross-add-or-update v-if="toupiaojiluCrossAddOrUpdateFlag" :parent="this" ref="toupiaojiluCrossaddOrUpdate"></toupiaojilu-cross-add-or-update>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="suodepiaoshuChart1" style="width:100%;height:600px;"></div>
...
import toupiaojiluCrossAddOrUpdate from "../toupiaojilu/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				toupiaojiluCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			toupiaojiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.toupiaojiluCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','toupiaohuodong');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.toupiaojiluCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.toupiaojiluCrossaddOrUpdate.init(row.id,type);
				});
			},
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var suodepiaoshuChart1 = echarts.init(document.getElementById("suodepiaoshuChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                suodepiaoshuChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    suodepiaoshuChart1.resize();
                };
            }
        });
      })
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.toupiaomingcheng!='' && this.searchForm.toupiaomingcheng!=undefined){
            params['toupiaomingcheng'] = '%' + this.searchForm.toupiaomingcheng + '%'
          }
    params['sort'] = 'suodepiaoshu';
    params['order'] = 'desc';
    params['sort'] = 'fabushijian';
    params['order'] = 'desc';
           if(this.searchForm.fabushijianstart!='' && this.searchForm.fabushijianstart!=undefined ){
            params['fabushijianstart'] = this.searchForm.fabushijianstart
          }
          if(this.searchForm.fabushijianend!='' && this.searchForm.fabushijianend!=undefined){
            params['fabushijianend'] = this.searchForm.fabushijianend
          }
			let user = JSON.parse(this.$storage.getObj('userForm'))
			console.log(user)
			this.$http({
				url: "toupiaohuodong/page",
				method: "get",
				params: params
			}).then(({ data }) => {
				if (data && data.code === 0) {
	...
	this.$router.push({path:'/discusstoupiaohuodong',query:{refid:id}});
    },
	toupiaohuodongstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'toupiaohuodong/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
